 ...
 *
 * Each Kapsel Extension and Agent MAY be assigned a W3C Decentralized Identifier (DID).
 * Full compliance hosts MUST assign DIDs for any cross-host interaction.
 *
 * Rules:
 *   - Extension and Agent DIDs resolvable via Kapsel Registry Protocol (§12)
...
/**
 * W3C DID Document fields for Kapsel entities.
 * @example did:kapsel:host-id:extension-id
 */
export interface KapselDIDDocument {
    /** W3C DID string, e.g. 'did:kapsel:host-id:extension-id' */
    did: string
    /** Public key for signing agent actions and authenticating */
    publicKey: string
    /** Communication endpoints */
    serviceEndpoints?: Record<string, string>
    /** URL that resolves to kapsel.json */
    kapselManifest?: string
 ...
 */
export interface KapselVerifiableCredential {
    /** Unique credential ID */
    id: string
    /** W3C VC type */
    type: 'VerifiableCredential' | 'KapselCapabilityCredential'
...
export interface SelectiveDisclosureResponse {
    ...
    credential: KapselVerifiableCredential
