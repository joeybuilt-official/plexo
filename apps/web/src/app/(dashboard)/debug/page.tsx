    ...
    return (
        <div className={`flex items-center justify-between rounded-lg border px-3 py-2.5 ${health.ok
            ...
            <div className="flex items-center gap-3">
                {health.error && (
                    <span className="text-xs text-red truncate max-w-[200px]">{health.error}</span>
                )}
                {health.latencyMs !== undefined && (
                    <span className="text-xs text-text-muted">{health.latencyMs}ms</span>
                )}
            </div>
        </div>
    )
...
function RouteRow({ check, result, wsId }: { check: RouteCheck; result: { ok: boolean; status: number; latencyMs: number } | null; wsId: string }) {
    ...
    return (
        <tr className="border-b border-border-subtle">
            ...
            <td className="py-2 pr-4 font-mono text-xs text-text-muted">{check.route}</td>
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-2 gap-4 xl:grid-cols-3">
                ...
                <div className="col-span-2 xl:col-span-1 rounded-xl border border-border bg-surface-1/40 p-4">
                    ...
                    {healthLoading ? (
                        ...
                        <p className="text-sm text-red">Health endpoint unreachable</p>
                    )}
                </div>
                ...
                <div className="col-span-2 rounded-xl border border-border bg-surface-1/40 p-4">
                    ...
                    <div className="h-36 overflow-y-auto rounded-lg bg-canvas p-3 font-mono text-[11px] text-text-muted">
                        {sseEvents.length === 0 ? (
                            ...
                            sseEvents.map((ev, i) => (
                                <div key={i} className="text-text-secondary">{ev}</div>
                            ))
                        )}
                    </div>
                    {!WS_ID && (
            ...
            <div className="rounded-xl border border-border bg-surface-1/40 overflow-hidden">
                ...
                >
                    ...
                    {envOpen ? <ChevronDown className="h-4 w-4 text-text-muted" /> : <ChevronRight className="h-4 w-4 text-text-muted" />}
                </button>
                {envOpen && (
